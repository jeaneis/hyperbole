...
\hl{connections to informal approaches, and limitations of the current model...} Our model has interesting connections to informal theories of communication and linguistic meaning. We formalized a notion of how the speaker's communicative goal and an assumption of optimal relevance together constrain the inferred meaning of an utterance to appropriate dimensions. By defining an affective dimension of meaning that is associated with a state of the world, we formalized a notion of linguistic meaning as including encyclopedic knowledge. Our model is currently limited to two dimensions of meaning and goals: the true state of the world and the speaker's affect, with affect simplified as a binary variable of either having or not having affect. In future work, we hope to extend our model to explain nonliteral language understanding more broadly by capturing the communicative goals in other figures of speech such as irony and metaphor. We believe that this framework significantly advances the flexibility and richness of formal models of language understanding, such that some day probabilistic models will explain \emph{everything} (hyperbolically speaking). 
...
\begin{materials}
\section{Model}
Let $u$ be the utterance a speaker utters. The meaning of the utterance has two dimensions: the actual price state $s$ and the speaker's affect $a$. We defined the set of price states $S=\{50, 51, 500, 501, 1000, 1001, 5000, 5001, 10000, 10001\}$. We assumed that the set of utterances $U$ is identical to $S$. We defined the set of affect states $A=\{0, 1\}$ (0 means no affect and 1 means with affect). Given the price states $S$ and affect states $A$, the set of possible meanings $M$ is given by $M = S \times A$. We denote each possible meaning as $s, a$, where $s \in S$ and $a \in A$. \hl{Attempted to fix.} A communicate goal $g$ also has two dimensions, the first concerning the price state, and the second concerning the speaker's affect. The speaker may have the goal of communicating information along the price dimension, the affect dimension, or both. If the speaker wants to communicate the price state dimension, she may want to communicate the price either exactly or approximately. When the speaker wants to communicate the price approximately, she represents numbers as their closest round neighbors. For example, such a speaker will represent the prices $51$ and $1001$ as $50$ and $1000$, respectively. Formally, a communicative goal $g$ can be expressed as the composition of two functions, $f$ and $r$. Depending on whether the speaker wants to communicate the price exactly or approximately, $f$ is defined as follows:
%
\[\left\{ 
  \begin{array}{l l}
    f_{e}(s) =  s \\
