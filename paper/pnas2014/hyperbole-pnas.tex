...
\begin{materials}
\section{Model}
Let $u$ be the utterance a speaker utters. The meaning of the utterance has two dimensions: the actual price state $s$ and the speaker's affect $a$. We defined the set of price states $S=\{50, 51, 500, 501, 1000, 1001, 5000, 5001, 10000, 10001\}$. We assumed that the set of utterances $U$ is identical to $S$. We defined the set of affect states $A=\{0, 1\}$ (0 means no affect and 1 means with affect). Given the price states $S$ and affect states $A$, the set of possible meanings $M$ is given by $M = S \times A$. We denote each possible meaning as $s, a$, where $s \in S$ and $a \in A$. \hl{Attempted to fix.} A communicative goal $g$ also has two dimensions, the first concerning the price state, and the second concerning the speaker's affect. The speaker may have the goal of communicating information along the price dimension, the affect dimension, or both. If the speaker wants to communicate the price state dimension, she may want to communicate the price either exactly or approximately. When the speaker wants to communicate the price approximately, she represents numbers as their closest round neighbors. For example, such a speaker will represent the prices $51$ and $1001$ as $50$ and $1000$, respectively. Formally, a communicative goal $g$ can be expressed as the composition of two functions, $f$ and $r$. Depending on whether the speaker wants to communicate the price exactly or approximately, $f$ is defined as follows:
%
\[\left\{ 
  \begin{array}{l l}
    f_{e}(s) =  s \\
