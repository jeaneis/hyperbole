...
%note: this cite should be:  http://www.sfs.uni-tuebingen.de/~gjaeger/publications/hskArticleGJ-2011.pdf o
...
$P(m)\propto b^{m} + c$. Parameters for the priors and cost of utterances are described in Figure \ref{fig:results}. We used $\lambda = 2$ and recursion depth $= 5$. Results are shown in Figure \ref{fig:results}. Note that since the model is restricted to eight discrete meanings, while subjects are free to choose whichever numeric interpretation they wish for each utterance, the scales for the $x$ axes on the plots are different. Subjects' interpretations are binned into widths of $50$ between the values of $0$ and $1050$. 
...
%\begin{figure}[t]
	...
	\end{subfigure}
        ...
        \caption{Each bar indicates probability of an utterance being interpreted as a certain meaning. Since the model is restricted to eight discrete meanings, while subjects' interpretations are continuous, the $x$ axes of the model and human plots are on different scales. Model's meaning priors: Textbook: $P(m) \propto (4 \times {10^{-4}})^m + 0.04$; Weather: $(2 \times {10^{-4}})^m$; Reading: $P(m) \propto (8 \times {10^{-5}})^m + 0.04$. Costs: round numbers = $ 0.25$; non-round numbers $= 0.286$. Subject's interpretations are taken from responses to Question (2) in Table 1, and categorized as having ``no affect"/ `affect" using a median split of subjects' responses to Question (3). Interpretations are binned with width $50$ for the plots.}
        \label{fig:results}
  \end{figure}
