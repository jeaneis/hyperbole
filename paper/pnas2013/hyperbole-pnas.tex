...
Given the incredible flexibility of language, a crucial part of a listener's job is to understand an utterance even when its literally meaning is extremely unlikely \cite{moreno2007creativity}. Non-literal language understanding is one of the biggest challenges in language research, and it has been difficult to build formal models or design empirical studies that capture effects in non-literal language understanding quantitatively. In this paper, we present a computational model that predicts people's non-literal interpretation of number words. We build on a traditional approach in language understanding that views communication as an interaction between rational, cooperative agents \cite{grice1975, clark1996using}, and show how non-literal interpretation of number terms can be explained as effects of probabilistic inference over recursive social models.
...
\begin{materials}
\section{Model}
Here we describe our model in detail. Let $u$ be the utterance a speaker utters. Let $m$ be the meaning interpretation a listener infers. $m$ has two dimensions, one concerning the actual price state $s$, and one concerning the speaker's affect $a$. Given the set of price states $S$ and set of affect states $A$, the set of possible meanings $M$ is given by $M = S X A$. We denote each possible meaning interpretation as $m_{sa}$, where $s \in S$ and $a \in \{0, 1\}$ ($0$ means no affect and $1$ means with affect). Let $g$ be the communicative goal, which also has two dimensions, one concerning the price state, and the other concerning the speaker's affect. We denote each communicative goal as $g_{pe}$ where $p \in \{s', s, 0 \}$ ($s'$ means communicate precise price state, $s$ means communicate imprecise price state, and $0$ means do not communicate price state) and $e \in \{a, 0\}$ ($a$ means communicate affect and $0$ means do not communicate affect). An interpreted meaning $m_{sa}$ satisfies the speaker's goal $g_{s'0}$ if $s$ is equal to the true price state and $a = 0$. $m_{s0}$ satisfies the goal $g_{s0}$ if $s$ is equal to the true price state $\pm 1$ and $a = 0$.$m_{sa}$ satisfies the goal $g_{0a}$ if $a = 1$. The probability of a price state $s$ is taken from an empirically derived price prior $P(s)$, and the probability of an affect given a price state $s$ is taken from an empirically derived conditional affect prior $E(s)$ (see Experiments 2a and 2b).
...
\[ L_0(m_{s,a} |u) = \left\{ 
  \begin{array}{l l}
    E(s) & \quad \text{if $s$ = $u$ and $a$ = 1}\\
    1 - E(s) & \quad \text{if $s$ = $u$ and $a$ = 0}\\
    0 & \quad \text{otherwise}
  \end{array} \right.\]
 % }
%\end{equation}

The speaker $S_n$ is assumed to be a rational planner who optimizes the probability that the listener will infer a meaning $m$ that satisfies the speaker's communicative goal while minimizing the cost of her utterance. $S_n$ chooses utterances according to a softmax decision rule that describes an approximately rational planner \cite{sutton1998reinforcement}:
...
The speaker wants to minimize both the cost $C(u)$ of the utterance and the surprisal of the intended meaning $m$ with respect to the communicative goal. Importantly, the information value of the intended meaning $m$ is $0$ if the communicative goal is not satisfied. So the utility function $U_n$ is defined by:
%\begin{equation}\label{eq:speakerutility}
\[ U_n(u | m, g)  = \left\{ 
  \begin{array}{l l}
    \log (L_{n}(m | u)) - C(u) & \quad \text{if $g$ satisfied}\\
    -C(u) & \quad \text{otherwise}
  \end{array} \right.\]
%\end{equation}
which combined with equation \ref{eq:speakerprob} leads to:
\begin{equation}\label{eq:speakersimplified}
S_n(u | m, g) \propto (L_{n}(m | u)e^{-c(u)}) ^\lambda.
\end{equation}
...
L_n(m|u) \propto \sum_{i }P(m)P(g)S_{n-1}(u | m,g).
\end{equation}

After obtaining a posterior distribution for all possible meanings $m$ given an utterance $u$, we performed a loose choice transformation on the distribution with $\lambda = 0.5$ in order to compare with behavioral results.
